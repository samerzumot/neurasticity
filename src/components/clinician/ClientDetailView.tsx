...
import { ClientProfile, ClinicBrandConfig, ExperienceType, ProtocolTemplate, ProtocolType, QEEGBrainMap, SessionRecord } from '../../types';
...
import { ArrowLeft, Send, Activity, Settings2, Download, Check, Upload, Brain, FileText, Sparkles } from 'lucide-react';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        >
          <ArrowLeft size={16} /> Back to Client Roster
        </button>
        ...
        <div style={{ display: 'flex', gap: '8px' }}>
          <button
            ...
            className="btn btn-ghost"
            style={{ border: '1px solid var(--border-default)', fontSize: '12px', padding: '6px 12px' }}
          >
            <Upload size={14} /> Import QEEG Brain Map
          </button>
          <button
          ...
          >
            <Download size={14} /> Download Clinical PDF Report
          </button>
        </div>
      </div>

      {/* Client Profile Header Card (Matching Spec §8.7) */}
      <div
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '13px', color: 'var(--text-secondary)', marginTop: '2px' }}>
              {client.condition} • Protocol: <strong>{client.assignedProtocol.replace(/-/g, ' ').toUpperCase()}</strong> • Last Session: {client.lastSessionDate}
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          <button
            ...
            className="btn btn-ghost"
            style={{ border: '1px solid var(--border-default)', fontSize: '13px', padding: '8px 14px' }}
          >
            <Settings2 size={15} /> Customize Protocol
          </button>
          <button onClick={onSendMessage} className="btn btn-secondary" style={{ padding: '8px 16px', fontSize: '13px' }}>
            <Send size={15} /> Message Client
          </button>
        </div>
      </div>
      ...
      {activeTab === 'eeg' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          <div className="card-clinician" style={{ padding: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              ...
              <div style={{ display: 'flex', gap: '14px', alignItems: 'center', fontSize: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <div style={{ width: '12px', height: '12px', borderRadius: '2px', background: 'var(--chart-delta)' }} />
                  <span>Delta (0.5-4Hz)</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <div className="pattern-theta" style={{ width: '12px', height: '12px', borderRadius: '2px' }} />
                  <span>Theta (4-8Hz)</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <div className="pattern-alpha" style={{ width: '12px', height: '12px', borderRadius: '2px' }} />
                  <span>Alpha (8-12Hz)</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <div className="pattern-beta" style={{ width: '12px', height: '12px', borderRadius: '2px' }} />
      ...
      {activeTab === 'protocol' && (
        <div className="card-clinician" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <h3 style={{ fontSize: '16px', fontWeight: 600, color: 'var(--text-primary)' }}>
                Active Treatment Protocol: {client.customProtocolConfig?.name || client.assignedProtocol.replace(/-/g, ' ').toUpperCase()}
              </h3>
              <p style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
                Montage Site: {client.customProtocolConfig?.montageSite || 'Fz / Cz'} | Session Length: {client.customProtocolConfig?.sessionDurationMinutes || 25} min
              </p>
            </div>
            <button onClick={() => setShowProtocolBuilder(true)} className="btn btn-dense">
              Switch or Edit Protocol Template
            </button>
          </div>

          <div className="card-patient-recessed" style={{ fontSize: '13px', lineHeight: 1.5 }}>
      ...
      {activeTab === 'brainmaps' && (
        <div className="card-clinician" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <h3 style={{ fontSize: '16px', fontWeight: 600 }}>In-Clinic QEEG Brain Maps & Scans</h3>
              <p style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
                19-channel quantitative EEG baseline data imported from clinical visitations.
              </p>
            </div>
            <button onClick={() => setShowBrainMapUpload(true)} className="btn btn-dense">
              <Upload size={14} /> Upload New Brain Map
            </button>
          </div>
          ...
          {client.brainMaps && client.brainMaps.length > 0 ? (
                ...
                >
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <span style={{ fontWeight: 600, fontSize: '14px' }}>{bm.deviceSource}</span>
                    </div>
                    <span style={{ fontSize: '12px', color: 'var(--text-tertiary)' }}>Imported {bm.uploadDate}</span>
                  </div>

                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '8px', margin: '12px 0' }}>
                    <div className="card-clinician" style={{ padding: '8px 12px' }}>
                      <div style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>Frontal Theta Z-Score</div>
                      <div className="font-mono" style={{ fontSize: '14px', fontWeight: 700, color: bm.zScores.frontalTheta > 2 ? 'var(--status-alert)' : 'var(--status-active)' }}>
                        Z = +{bm.zScores.frontalTheta}
                      </div>
                    </div>
                    <div className="card-clinician" style={{ padding: '8px 12px' }}>
                      <div style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>Central Beta Z-Score</div>
                    ...
                    <div className="card-clinician" style={{ padding: '8px 12px' }}>
                      <div style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>Occipital Alpha Z-Score</div>
                      <div className="font-mono" style={{ fontSize: '14px', fontWeight: 700 }}>
                        Z = {bm.zScores.occipitalAlpha >= 0 ? `+${bm.zScores.occipitalAlpha}` : bm.zScores.occipitalAlpha}
                      </div>
                    </div>
                    <div className="card-clinician" style={{ padding: '8px 12px' }}>
                      <div style={{ fontSize: '11px', color: 'var(--text-secondary)' }}>Dominant Alpha Peak (IAF)</div>
                      <div className="font-mono" style={{ fontSize: '14px', fontWeight: 700, color: 'var(--chart-alpha)' }}>
                  ...
                  <div style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
                    <strong>Technician Impressions:</strong> {bm.technicianNotes}
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ padding: '32px', textAlign: 'center', color: 'var(--text-tertiary)' }}>
              No in-clinic brain maps uploaded yet. Click "Upload New Brain Map" to import an EDF or QEEG report.
            </div>
          )}
        </div>
      )}
      ...
      {activeTab === 'telemetry' && (
        <div className="card-clinician" style={{ padding: '24px', textAlign: 'center' }}>
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '8px', color: 'var(--status-active)', marginBottom: '8px' }}>
            <Activity size={18} />
            <span style={{ fontWeight: 600 }}>Remote EEG Stream Live Feed</span>
          </div>
          <div
            style={{
              width: '100%',
          ...
          >
            ...
            <div style={{ position: 'absolute', top: 12, right: 12, color: '#FFFFFF', fontSize: '11px', fontFamily: 'var(--font-mono)' }}>
              100 Hz Sampling • 4 Channels (TP9, AF7, AF8, TP10) Verified
            </div>
          </div>
        </div>
      )}
      ...
      {activeTab === 'sessions' && (
        <div className="card-clinician" style={{ padding: '20px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
            <h3 style={{ fontSize: '16px', fontWeight: 600 }}>Archived Session Time-Series Data</h3>
            <button onClick={handleDownloadPDF} className="btn btn-dense" style={{ fontSize: '12px' }}>
            ...
            {sessions.map(s => (
              ...
              >
                <div>
                  ...
                  <div style={{ fontSize: '12px', color: 'var(--text-secondary)' }}>
                    Duration: {Math.round(s.durationSeconds / 60)} min | In-Zone: {s.timeInZonePercent}% | Peak Focus: {s.peakFocusScore}
                  </div>
                </div>
                <button onClick={handleDownloadPDF} className="btn btn-ghost" style={{ fontSize: '12px', padding: '4px 8px' }}>
