...
export const PatientAppointmentsView: React.FC<PatientAppointmentsViewProps> = ({ repository = appointmentRepository }) => {
  ...
  const load = useCallback(async () => {
    ...
    try { setAppointments(await repository.list('patient')); setState('ready'); }
  ...
  return <section aria-label="Appointments" style={{ display: 'grid', gap: 12 }}><h1 style={{ margin: 0, fontSize: 22 }}>Appointments</h1>{appointments.map((appointment) => <article key={appointment.id} className="patient-card" style={{ padding: 16, opacity: appointment.status === 'cancelled' ? 0.68 : 1 }}><div style={{ fontWeight: 650, textTransform: 'capitalize' }}>{appointment.type.replaceAll('-', ' ')}</div><div style={{ display: 'flex', gap: 6, alignItems: 'center', fontSize: 13, marginTop: 6 }}><Clock size={14} /> {formatAppointmentDateTime(appointment.startsAtMillis, appointment.timezone)} · {appointment.durationMinutes} min</div><div style={{ fontSize: 12, marginTop: 5 }}>{appointment.timezone} · <span style={{ textTransform: 'capitalize' }}>{appointment.status}</span></div>{appointment.clinicianDisplayName && <div style={{ fontSize: 12, marginTop: 5 }}>With {appointment.clinicianDisplayName}</div>}{appointment.notes && <div style={{ fontSize: 12, marginTop: 8, opacity: 0.75 }}>{appointment.notes}</div>}</article>)}</section>;
};
